...
function getFilePath($file_path) {
    ...
    return '../admin/uploads/documents/' . $filename;
}
            ...
            <div class="documents-grid">
                ...
                <?php else: ?>
                    <?php foreach ($all_documents as $document): 
                        $file_path = getFilePath($document['file_path']);
                        $file_exists = checkFileExists($document['file_path']);
                    ?>
                        <div class="document-card">
                            <div class="preview-container">
                                <?php 
                                $extension = strtolower(pathinfo($document['file_name'], PATHINFO_EXTENSION));
                                if (in_array($extension, ['jpg', 'jpeg', 'png', 'gif']) && $file_exists): 
                                ?>
                                ...
                                <div class="file-status <?php echo $file_exists ? 'file-available' : 'file-missing-status'; ?>">
                                    <?php echo $file_exists ? 'Available' : 'Missing'; ?>
                                </div>
                            </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="file-info <?php echo !$file_exists ? 'file-missing' : ''; ?>">
                                    <i class="fas fa-file"></i>
                                    <?php echo htmlspecialchars($document['file_name']); ?>
                                    (<?php echo round($document['file_size'] / 1024 / 1024, 2); ?> MB)
                                    <?php if (!$file_exists): ?>
                                        <br><small class="file-missing">File not found on server</small>
                                    <?php endif; ?>
                                </div>
                            </div>
